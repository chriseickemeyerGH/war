...
function App() {
  ...
  const userWin = async (
    ...
    try {
      ...
      setTimeout(() => {
        ...
        afterWinActions(true, warAgainCpuRemaining, winningMessage);
      }, 2000);
    } catch (err) {
      onError(err);
    }
  ...
  const cpuWin = async (
    ...
    try {
      ...
      setTimeout(() => {
        ...
        afterWinActions(true, warAgainUserRemaining, losingMessage);
      }, 2000);
    } catch (err) {
      onError(err);
    }
  ...
  const warDraw = async (
    ...
    try {
      ...
      const userCardTwo = setCardObject(image, value, suit, code, false);
      ...
      setTimeout(() => {
        if (
          cardValue(userSecondCard.value) === cardValue(cpuSecondCard.value)
        ) {
          warActions(warAgainUserRemaining, warAgainCpuRemaining, warDraw);
        } else {
          const cpuCodesArr = [],
            userCodesArr = [];
  ...
  const normalUserWin = async (bothCardCodes, cpuCardsRemaining) => {
    try {
      const res = await axios.get(
  ...
  const normalCPUWin = async (bothCardCodes, userCardsRemaining) => {
    try {
      const res = await axios.get(
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (cardValue(userCard.value) === cardValue(cpuCard.value)) {
          ...
          normalUserWin(bothCardCodes, cpuCardsRemaining);
        } else if (cardValue(userCard.value) < cardValue(cpuCard.value)) {
          normalCPUWin(bothCardCodes, userCardsRemaining);
        }
      }, 1600);
    } catch (err) {
      onError(err);
  ...
  return (
    <>
      <GlobalStyle />
      {decksNotSet && <Instructions onStart={createDecks} />}
      {showGameVals && (
        <GameTable
          cardsLeft={cardsLeft}
          cardDrawn={cardDrawn}
